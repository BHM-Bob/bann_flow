...
Date: 2023-05-21 12:57:34
LastEditors: BHM-Bob 2262029386@qq.com
LastEditTime: 2023-05-21 13:07:05
...
try:
    from bann_flow.utils import Mprint, GlobalSettings
    import bann_flow.bb as bb
    import bann_flow.m as m
except:
    from utils import Mprint, GlobalSettings
    import bb
    import m

criterion = nn.CrossEntropyLoss().cuda('cuda')
label = flow.randint(0, 128, size = (32, )).to('cuda')
        
x = flow.rand([32, 32, 1024], device = 'cuda')

net = m.COneDLayer(m.LayerCfg(32, 64, 3, 2, 'SABlock1D', avg_size=4)).to('cuda')
print(net(x).shape, "flow.Size([32, 64, 256])")
...
net = m.COneDLayer(m.LayerCfg(32, 64, 3, 2, 'SABlock1DR', avg_size=4,
                              ...
                              trans_cfg=m.TransCfg(64, use_FastMHA = False))).to('cuda')
print(net(x).shape, "flow.Size([32, 64, 256])\n")


x = flow.rand([32, 32, 64, 64], device = 'cuda')

net = m.MAlayer(m.LayerCfg(32, 64, 3, 2, 'ResBlock', 'SABlock')).to('cuda')
print(net(x).shape, "flow.Size([32, 64, 32, 32])\n")
...
x = flow.rand([32, 3, 128, 128], device = 'cuda')
cfg = [
...
print('MATTPBase', logits.shape, "flow.Size([32, 128])\n")


x = flow.rand([32, 8, 1024], device = 'cuda')
cfg = [
...
logits = net(x)
criterion(logits, label).backward()
print('COneD', logits.shape, "flow.Size([32, 128])\n")
...
logits = net(x)
criterion(logits, label).backward()
print('MATTP', net(x).shape, "flow.Size([32, 256, 128])\n")
...
print('MAvTTP', net(x).shape, "flow.Size([32, 128])\n")
...
print('MATTPE', net(x).shape, "flow.Size([32, 128])\n")
...
# print('SCANNTTP', net(x).shape, "flow.Size([32, 256, 128])\n")
...
print('MATTP_ViT', net(x).shape, "flow.Size([32, 258, 128])\n")
